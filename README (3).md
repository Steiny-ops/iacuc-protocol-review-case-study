...
- **Automation:** Google Apps Script attached to the Sheet for the jobs AppSheet cannot do: normalizing form-submission intake into the protocol table, and a nightly timestamped backup.

Seven tables: Protocols, Users, Assignments, Reviews, RevisionRequests, PIResponses, CoordinatorNotes. Users drives everything: a Role column (Coordinator, Reviewer, PI, Chair) that every security filter and action condition looks up by the signed-in email.

## The role model

Four roles, four very different systems:

| Role | Sees | Can do |
|---|---|---|
| Coordinator | Everything | Maintains all data, assigns reviewers, resolves revision rounds, keeps private notes |
...
| Researcher (PI) | Only their own protocols; only revision requests actually sent to them | Edits application content pre-review and during revisions; submits immutable responses |
| Chair | Everything, for oversight | Can enter a new protocol record; alters nothing else |

Visibility is enforced by security filters (rows a role cannot see are never loaded to their device, data-level privacy, not hidden UI). Mutation is enforced separately by action conditions and column-level editability. Keeping those two layers distinct was one of the central lessons: **visibility rules and mutation rules fail independently, and auditing one tells you nothing about the other.**
...
3. **Reviewed content is stable.** The researcher can edit their application only before review starts or while revisions are open. During review, content freezes, so signatures always refer to the text that was actually reviewed.
...
Statuses are earned, not remembered. Four bots:

- Assigning a reviewer advances a protocol to In Review.
- Sending a revision request advances it to Revisions Requested.
- Signing a review, or resolving a revision request, triggers a completion check: when every review is signed and every sent request is resolved, the protocol advances itself to the terminal review status.
